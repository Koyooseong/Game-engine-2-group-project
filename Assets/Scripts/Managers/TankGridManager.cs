...
public class TankGridManager : MonoBehaviour
{
    ...
    public void RegisterGrid(Vector2Int position, TankGridController controller)
    {
        ...
        Log.System($"[TankGridManager] 셀 등록됨: {position} | 해금 여부: {controller.IsUnlocked}", controller);
    }
    ...
    public bool CanPlaceBlock(Vector2Int anchor, Vector2Int[] shape)
    {
        if (shape == null || shape.Length == 0)
        {
            Log.Error("[CanPlaceBlock] shape이 null이거나 비어있음", this);
            return false;
        }
        ...
        foreach (var offset in shape)
        {
            ...
            if (!gridMap.TryGetValue(pos, out var cell))
            {
                Log.Warn($"[CanPlaceBlock] 셀 없음: {pos}");
                return false;
            }

            if (!cell.IsUnlocked)
            {
                Log.Warn($"[CanPlaceBlock] 잠금 셀: {pos}");
                return false;
            }

            if (cell.HasBlock)
            {
                Log.Warn($"[CanPlaceBlock] 이미 블록 있음: {pos}");
                return false;
            }
        }
